    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        if (isFirst && mDatas != null && mDatas.size() > 0) {
            ...
            enterAnimator.addListener(new AnimatorListenerAdapter() {
                ...
                @Override
                public void onAnimationEnd(Animator animation) {
                    Log.e("noah", "onAnimationEnd");
                    enterAnimator = null;
                    if (currentView instanceof ScaleImageView){
                        currentView.postDelayed(new Runnable() {
                            @Override
                            public void run() {
                                ScaleImageView imageView = (ScaleImageView) ImageWatcher.this.currentView;
                                imageView.onSimulationZoom(ScaleImageView.ZOOM_FOCUS_CENTER,imageView.getMaxWidth() / 2, 0);
                            }
                        },50);
                    }
                }
        ...
        private ScaleImageView disposeImageView(FrameLayout frameLayout, int position) {
            ...
            imageView.setOnImageEventListener(new ScaleImageView.DefaultOnImageEventListener() {
                ...
                public void onImageLoaded() {
                    imageView.onSimulationZoom(0,imageView.getMaxWidth() / 2, 0);
